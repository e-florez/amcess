...
def test_str():
    ...
    """
    w = Cluster({"atoms": [("H", 0, 0, 0)]}, [("H", 0, 0, 0), ("H", 0, 0, 0)])
    w_str = str(w)
...
def test_GetMol_Fails():
    ...
    assert str(e.value) == (
        f"\nCluster with {mol.GetNumMols()} total molecules "
        f"and index [0-{mol.GetNumMols() - 1}]"
        f"\n atom index must be less than {mol.GetNumMols()}"
        f"\nCheck! You want to get atom with index {100}"
    )
...
def test_InitializeCluster():
    ...
    for i in w3_no_overlapping.GetClusterDict():
        ...
        overlap.append(
            Cluster().Overlapping(
                mol1.GetMolCoord(), mol2.GetMolCoord()
            )
        )
...
def test_RotateMol(angles, expected_rotation):
    ...
    assert np.allclose(
        mol_rotated.GetMolCoord(),
        expected_mol.GetMolCoord(),
        0.01,
    )
...
def test_RotateMol_FreezeMol():
    ...
    print(mol.GetMolCoord()[3], mol_rotated.GetMolCoord()[3])

    assert (
        mol.GetMolCoord()[3]
        == mol_rotated.GetMolCoord()[3]
    )
...
def test_RotateMol_SingleAtom():
    ...
    assert mol.GetMolCoord() == mol_rotated.GetMolCoord()
...
def test_CalCenterRSphere_TE(molecule1, molecule2):
    ...
    with pytest.raises(TypeError) as e:
        ...
        obj_cluster.CalCentRSphere(tolerance_radius=[1])
